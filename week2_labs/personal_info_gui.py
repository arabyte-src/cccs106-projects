...
def main(page: ft.Page):
    ...
    # Title
    title = ft.Text(
        "Personal Information Manager".upper(),
        size=28,
    ...
    # Input fields
    first_name = ft.TextField(label="First Name", width=280, expand=True)
    last_name = ft.TextField(label="Last Name", width=280, expand=True)
    age = ft.TextField(label="Age", width=100, keyboard_type=ft.KeyboardType.NUMBER, expand=True)
    student_id = ft.TextField(label="Student ID", width=200, expand=True)
    spacing=20
    ...
    hobbies = ft.TextField(label="Hobbies/Interests", width=450, multiline=True)
    ...
    def generate_profile(e):
        try:
            ...
            profile_content = ft.Column([
                ft.Text("🎓 STUDENT PROFILE", size=20, weight=ft.FontWeight.BOLD, color=ft.Colors.INDIGO_700, font_family='Roboto Condensed Black'),
                ft.Divider(),
                ...
                ft.Divider(),
                ft.Text(f"🎓 Expected Graduation: {graduation_year}", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.INDIGO_700),
